...
"""
app/dashboard.py — standalone version (no separate backend process needed)
"""

import sys
from pathlib import Path

# Streamlit runs this file directly, which only adds app/ to sys.path,
# not the project root — so "from app.config import ..." would fail
# without this. Add the project root manually.
...
<style>
    #MainMenu, footer, header {visibility: hidden;}
    .block-container {padding-top: 2.2rem; max-width: 1200px;}
    .aqi-card { border-radius: 16px; padding: 20px 16px; text-align: center;
        background: var(--card-bg); border: 1px solid var(--card-border); box-shadow: 0 4px 14px rgba(0,0,0,0.25); }
    .aqi-card .label { font-size: 0.8rem; color: #9ca3af; text-transform: uppercase; letter-spacing: 0.04em; margin-bottom: 6px; }
    .aqi-card .value { font-size: 2rem; font-weight: 700; line-height: 1.1; margin-bottom: 8px; }
    .aqi-card .badge { display: inline-block; padding: 3px 12px; border-radius: 999px; font-size: 0.72rem; font-weight: 600; }
    .alert-banner { border-radius: 12px; padding: 14px 18px; margin-bottom: 18px; font-weight: 600; font-size: 0.95rem; }
    .alert-warning { background: rgba(255, 126, 0, 0.12); border: 1px solid rgba(255, 126, 0, 0.5); color: #ffb066; }
    .alert-danger { background: rgba(220, 38, 38, 0.15); border: 1px solid rgba(220, 38, 38, 0.6); color: #ff8080; }
    h1 {font-size: 1.7rem !important; margin-bottom: 0.2rem !important;}
    div[data-testid="stExpander"] { border-radius: 14px; border: 1px solid #262b36; }
</style>
""", unsafe_allow_html=True)


@st.cache_data(ttl=60)
def cached_predict(overrides_key=None, overrides=None):
    predictions, X = predict_all_horizons(overrides)
...
def format_predictions(predictions):
    ...
    for target, value in predictions.items():
        ...
        out[target] = {"predicted_aqi": round(value, 2), "category": label, "color": color}
    return out


def render_cards(predictions, size="large"):
    value_size = "2rem" if size == "large" else "1.4rem"
    cols = st.columns(3)
    for col, target in zip(cols, TARGETS):
        p = predictions[target]
...
def render_alerts(predictions, context=""):
    ...
    if worst_severity:
        prefix = f"{context} — " if context else ""
        css_class = "alert-danger" if worst_severity == "danger" else "alert-warning"
        st.markdown(f'<div class="alert-banner {css_class}">{prefix}Hazardous air quality predicted: {" · ".join(messages)}</div>', unsafe_allow_html=True)
...
predictions_raw, X_latest = cached_predict()
predictions = format_predictions(predictions_raw)
render_alerts(predictions)

hist_df = get_history(history_days)

fig = go.Figure()
fig.add_trace(go.Scatter(x=hist_df["date"], y=hist_df["AQI"], mode="lines",
                          line=dict(color="#60a5fa", width=2.5, shape="spline", smoothing=0.5),
...
fig.update_layout(height=280, margin=dict(t=10, b=10, l=10, r=10),
                   ...
                   yaxis=dict(showgrid=True, gridcolor="rgba(255,255,255,0.06)", title=None, zeroline=False),
                   plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
                   font=dict(color="#9ca3af"), hovermode="x unified")
st.plotly_chart(fig, use_container_width=True)
...
with st.expander("Adjust conditions"):
    ...
    overrides = {"Temperature": temp, "PM2.5": pm25, "Humidity": humidity}
    if st.button("Update forecast"):
        whatif_raw, whatif_X = predict_all_horizons(overrides)
        st.session_state["whatif_predictions"] = format_predictions(whatif_raw)
        st.session_state["last_overrides"] = overrides
...
with tab_local:
    overrides = st.session_state.get("last_overrides")
    if overrides:
        _, X_for_explain = predict_all_horizons(overrides)
    else:
        X_for_explain = X_latest
    explanation = explain_row(horizon, X_for_explain)

    contrib_df = pd.DataFrame(explanation["contributions"]).head(12).sort_values("shap_value")
    colors = ["#ef4444" if v > 0 else "#22c55e" for v in contrib_df["shap_value"]]
    fig2 = go.Figure(go.Bar(x=contrib_df["shap_value"], y=contrib_df["feature"], orientation="h",
                             marker_color=colors, hovertemplate="%{y}: %{x:.2f}<extra></extra>"))
    fig2.update_layout(height=420, margin=dict(t=10, b=10, l=10, r=10),
                        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#9ca3af"),
                        xaxis=dict(title="Impact on predicted AQI", gridcolor="rgba(255,255,255,0.06)"), yaxis=dict(title=None))
    st.plotly_chart(fig2, use_container_width=True)
    st.caption("🔴 increases predicted AQI · 🟢 decreases predicted AQI")

with tab_global:
    global_exp = explain_global(horizon, sample_size=100)
    imp_df = pd.DataFrame(global_exp["importance"]).head(12).sort_values("mean_abs_shap")
    fig3 = go.Figure(go.Bar(x=imp_df["mean_abs_shap"], y=imp_df["feature"], orientation="h",
                             marker_color="#60a5fa", hovertemplate="%{y}: %{x:.2f}<extra></extra>"))
    fig3.update_layout(height=420, margin=dict(t=10, b=10, l=10, r=10),
                        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#9ca3af"),
                        xaxis=dict(title="Average impact", gridcolor="rgba(255,255,255,0.06)"), yaxis=dict(title=None))
    st.plotly_chart(fig3, use_container_width=True)